<template>
              ...
              <button class="btn dropdown-toggle btn-primary" type="button" data-bs-toggle="dropdown" aria-expanded="false">Actions</button>
                  <ul class="dropdown-menu dropdown-block" style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate(267px, 305px);" data-popper-placement="bottom-start">
                    ...
                    <li class="dropdown-item d-flex align-items-center">
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="javascript:void(0);" @click="suppression(typesDepenses.id,typesDepenses,'typeDepenses','le typeDepense')"
                      >
                        <i
  ...
  <AddTypesDepensesModal :item="selectedItem" @close="recharger"/>
...
export default defineComponent({
  ...
  setup(){
    ...
    const deleteTypesDepenses = (id: number) => {
      ApiService.delete(`/typesDepenses/${id}`)
      .then(({ data }) => {
        Swal.fire({
          text: data.message,
          toast: true,
      ...
      for(let i = 0; i < typesDepenses.value.length; i++) {
        if (typesDepenses.value[i].id === id) {
          typesDepenses.value.splice(i, 1);
        }
      }
    };
