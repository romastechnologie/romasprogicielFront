<template>
    <div class="modal fade" id="AddCategoriesDepensesModal" tabindex="-1" role="dialog" ref="addCategoriesDepensesModalRef"
...
export default {
    ...
    setup: (props: any, { emit }: { emit: Function }) => {
        ...
        const getAllCategoriesDepenses = async () => {
            try {
                ...
                categoriesDepensesOptions.value = categoriesDepensesData.map((categoriesDepenses) => ({
                    ...
                    label: categoriesDepenses.nomBien,
                }));
            }
            catch (error) {
                //error(response.data.message)
        ...
        return {
            categoriesDepenses, title, btntext, resetValue, categoriesDepensesSchema,
