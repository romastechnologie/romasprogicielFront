...
//déclaration des champs
    ...
    const demandenew = ref(props.id);
    const demandeForm =  ref<Demande | null>(null);
    const addDemandeModalRef = ref<null | HTMLElement>(null);
    let demandes= ref<Array<Demande>>([]);
    ...
    const getDemande = async (id: number) => {
      ...
      .then(({ data }) => {
        demandeForm.value?.setFieldValue("id",data.data.id);
        demandeForm.value?.setFieldValue("libelle",data.data.libelle);
        demandeForm.value?.setFieldValue("description",data.data.description);
        emit('openmodal', addDemandeModalRef.value);
      })
      .catch(({ response }) => {
...
return {demandes, title,btntext, resetValue, schemaDemande,
       addDemande, demandeForm,addDemandeModalRef,demandenew,categorieDemandeChange,fieldHide1,
      fieldHide2,
      fieldHide3,
      fieldHide4,
      fieldHide5,
