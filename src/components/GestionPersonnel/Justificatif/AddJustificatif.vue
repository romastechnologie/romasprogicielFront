<template>
    ...
    <div class="card-body p-15 p-sm-20 p-md-25 p-lg-30 letter-spacing">
          ...
          <div class="col-md-4 mb-3">
            <div class="form-group mb-15 mb-sm-20 mb-md-25">
              ...
              <Field name="preuveFileName" type="file" class="form-control shadow-none fs-md-15 text-black" />
  ...
  export default defineComponent({
    ...
    preuveFileName: Yup.string().required("La preuve est obligatoire."), 
  });
      ...
      const addJustificatif = async (values: any, { resetForm }) => {
      values['personnel'] = personnel.value
      console.log('Données envoyées', values)
      if (showMErr.value === false) {
        ApiService.post("/justificatifs", values)
           .then(({ data }) => {
             if (data.code == 201) {
              success(data.message);
               //resetForm();
             console.log('flefelef')
              router.push({ name: "ListeJustificatif" });
           }
           }).catch(({ response }) => {
      ...
      return {justificatifSchema , addJustificatif, justificatifForm,personnelOptions,personnel};
