              ...
              <ul class="dropdown-menu dropdown-block" style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate(267px, 305px);" data-popper-placement="bottom-start">
                ...
                <li class="dropdown-item d-flex align-items-center">
                  <a href="javascript:void(0);"
                      @click="suppression(categorie.id,categories,'categorieDemandes',`la categorie ${categorie.libelle}`)">  <i class="fa fa-trash-o lh-2 me-8 position-relative top-1"></i>
                       Supprimer
                  </a>
                </li>
              </ul>
...
export default defineComponent({
  ...
  setup: () => {
    ...
    function getAllCategories(page = 1, limi = 10, searchTerm = '') {
      return ApiService.get(`all/categorieDemandes?page=${page}&limit=${limi}&mot=${searchTerm}&`)
        .then(({ data }) => {
