...
export default defineComponent({
  ...
  setup: () => {
    ...
    const addHoraire = async () => {
       ApiService.post("/horaires", horaires)
        .then(({ data }) => {
          if (data.code == 201) {
    ...
    onMounted(async () => {
      for (let index = 0; index < jours.length; index++) {
        horaires.push({
          jour: jours[index],
          heureOuverture: (index > 4)?"":"08:00",
          heureFermeture: (index > 4)?"":"18:30",
          heureDebutPause: (index > 4)?"":"13:00",
          heureFinPause:  (index > 4)?"":"15:00",
          estActif: (index > 4)? 0:1
        });
      }
    })
