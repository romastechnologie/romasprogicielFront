<template>
  ...
  <div class="card-body p-15 p-sm-20 p-md-25 p-lg-30 letter-spacing">
          <Form ref="mediaForm" @submit="editMedia" :validation-schema="mediaSchema" :initial-values="mediaForm">
          <div class="row">
            <div class="col-md-6">
...
export default defineComponent({
  ...
  setup: () => {
    const mediaSchema = Yup.object().shape({
      ...
      nomMedia: Yup.string().required('Le nm du média est obligatoire'),
      typeMedia: Yup.string().required('Le type de média est obligatoire'),
      extension: Yup.string().required('L\'extension est obligatoire'),
    ...
    function getMedia(id:number) {
      ApiService.get("/medias/"+id.toString())
        .then(({ data }) => {
          for (const key in data.data) {
            mediaForm.value?.setFieldValue(key, 
    ...
    const editMedia = async (values, {resetForm}) => {
      const data = mediaForm.value;
      ApiService.put("/medias/" + data?.id, data)
        .then(({ data }) => {
          if (data.code == 200) {
    ...
    onMounted(() => {
      if(route.params.id) {
        getMedia(parseInt(route.params.id as string));
      }
    });

    return { 
      mediaSchema, editMedia, mediaForm, typeMediaOptions
    };
  },
});
</script>