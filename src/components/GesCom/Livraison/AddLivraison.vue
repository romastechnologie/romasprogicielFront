<template>
  <div class="card mb-25 border-0 rounded-0 bg-white add-user-card">
    <div class="card-body p-15 p-sm-20 p-md-25 p-lg-30 letter-spacing">
      <Form
        @submit="addLivraison"
        :validation-schema="livraisonSchema"
        :keep-values="valuess"
      >
        <div class="row">
          <div class="col-md-4">
            ...
            <ErrorMessage name="dateLivr" class="text-danger" />
          </div>
          <div class="col-md-4">
            <div class="form-group mb-15 mb-sm-20 mb-md-25">
              <label class="d-block text-black fw-semibold mb-10">
                                ...
                                <div class="row mt-10 marge-droite">
                                  ...
                                  <div class="col-md-2">
                                    <label
                                      class="d-block text-black fw-semibold mb-10"
                                    >
                                      Prix de revient<span class="text-danger"
                                        >*</span
                                      >
                                    </label>
                                  </div>
                                  ...
                                  <div class="col-md-2">
                                    ...
                                    >
                                      <input
                                        ...
                                        type="number"
                                        :class="
                                          validateRowProduit(produit.prixRevient)
                                            ? 'form-control shadow-none fs-md-15 text-black is-invalid '
                                            : 'form-control shadow-none fs-md-15 text-black '
                                        "
          ...
          <div class="col-md-12 mt-3">
            <div class="d-flex align-items-center">
              ...
              <router-link
                to="/livraisons/liste-livraisons"
...
export default defineComponent({
  ...
  setup: () => {
    const livraisonSchema = Yup.object().shape({
      dateLivr: Yup.date().required("La date est obligatoire"),
      commande: Yup.string().required("La commande est obligatoire"),
      livreur: Yup.string().required("Le livreur est obligatoire"),
    });

    const livraisonForm = ref<Livraison | null>(null);
    const router = useRouter();
    const commandeOptions = ref([]);
...
const validateRowProduit = (e) => {
  ...
  { deep: true }
    ...
    const addLivraison = async (values, { resetForm }) => {
      console.log('Dedans')
      ...
      // ApiService.post("/livraisons", values)
      //   .then(({ data }) => {
      //     if (data.code == 201) {
      //       success(data.message);
      //       resetForm();
      //       router.push({ name: "ListeLivraisonPage" });
      //     }
      //   })
      //   .catch(({ response }) => {
    ...
    const fetchFournisseur = async () => {
      try {
        const response = await ApiService.get("all/commandes");
        const commandeData = response.data.data.data;
        commandeOptions.value = commandeData.map((commande) => ({
          value: commande.id,
          label: `${
            commande.nomFournisseur
              ? commande.nomFournisseur
              : commande.denomination
          }`,
        }));
      } catch (error) {
        //
