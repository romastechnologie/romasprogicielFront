...
setup: () => {
  const operationSchema = Yup.object().shape({
    ...
    typeOperation: Yup.string().required('La nature de l\'operation est obligatoire'),
  ...
  const addOperation = async (values, { resetForm }) => {
    ...
    //  values.nature = "Débit";
    ApiService.post("/decaissements", values)
      .then(({ data }) => {
       if (data.code == 200) { 
  ...
  const fetchComptes = async () => {
      ...
      const fetchTypeOperations = async () => {
          try {
              ...
              const encaissementOperations = typeoperationsData.filter(typeoperation => typeoperation.nature === "Débit");
