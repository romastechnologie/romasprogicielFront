<template>
    ...
    >
      <div class="d-sm-flex align-items-center">
        <a 
          ...
          data-bs-target="#AddTypeBienModal"
        >
        ...
        <table class="table text-nowrap align-middle mb-0">
          ...
          <tbody>
            <tr v-for="(typeBien, index) in typeBiens" :key="index">
              ...
              <td class="shadow-none lh-1 fw-medium text-black-emphasis">
                {{ format_date(typeBien.createdAt)  }}
              </td>
              <td
              ...
              <ul class="dropdown-menu dropdown-block" style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate(267px, 305px);" data-popper-placement="bottom-start">
                <li class="dropdown-item d-flex align-items-center">
                  <a  href="javascript:void(0);" @click="moddifier(typeBien)">
                  <i class="fa fa-pencil lh-2 me-8 position-relative top-1"></i> Modifier
                  </a>
                </li>
                <li class="dropdown-item d-flex align-items-center">
                  <a href="javascript:void(0);"
                      @click="suppression(typeBien.id,typeBiens,'typeBiens',`le typeBien ${typeBien.libelle}`)">  <i class="fa fa-trash-o lh-2 me-8 position-relative top-1"></i>
                       Supprimer
                  </a>
                </li>
              </ul>
  ...
  <AddTypeBienModal
    @get-all-typeBiens="getAllTypeBiens"
    :id="idtypeBien"
    @openmodal="showModalEdite"
    @close="recharger"
    @refreshTypeBiens="refreshTypeBiens"
  />
</template>
<script lang="ts">
import { defineComponent, onMounted, ref  } from "vue";
import AddTypeBienModal from "./AddTypeBienModal.vue";
...
export default defineComponent({
  ...
  setup: () => {
    ...
    const handlePaginate = ({ page_, limit_ }: { page_: number, limit_: number }) => {
     ...
     function rechercher(){
      getAllTypeBiens(page.value, limit.value, searchTerm.value );
    }
    ...
    function moddifier(EdittypeBien:TypeBien) {
      idtypeBien.value = EdittypeBien.id;
    }
    ...
    const checkTypeBien = (name:string) => {
