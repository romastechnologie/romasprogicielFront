<template>
    ...
    <div class="card-body p-15 p-sm-20 p-md-25 p-lg-30 letter-spacing">
            <Form ref="courseForm" @submit="editCourse" :validation-schema="courseSchema">
              <div class="row">
              <div class="col-md-4">
  ...
  export default defineComponent({
    ...
    setup: () => {
      ...
      onMounted(() => {
        ...
        getAllBiens();
        if(route.params.id) {
        getCourse(parseInt(route.params.id as string));
      }
      });
      ...
      ApiService.get("/sortie/"+id.toString())
        .then(({ data }) => {
    ...
    const editCourse = async (values, {resetForm}) => {
      ApiService.put("/sortie/"+values.id,values)
        .then(({ data }) => {
          if(data.code == 200) { 
    ...
    const getAllPersonnels = async () => {
      ...
      return { courseSchema, editCourse, courseForm,bienOptions,showMErr,categorieOptions,personnels,biens,personnelOptions};
    },
  });
  </script>