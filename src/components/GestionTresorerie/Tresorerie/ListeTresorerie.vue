<template>
      ...
      >
        <div class="d-sm-flex align-items-center">
          <router-link
            ...
            to="/tresoreries/ajouter-tresorerie"
          >
          ...
          <table class="table text-nowrap align-middle mb-0">
            ...
            <tbody>
              ...
              <td class="shadow-none lh-1 fw-medium text-black-emphasis">
                <span v-if="tresorerie.operation == 1">
                  oui
                </span>
                <span v-if="tresorerie.operation == 0">
                  non 
                </span>
              </td>
                ...
                <div class="dropdown">
                  <button class="btn dropdown-toggle btn-primary" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                    ...
                    <ul class="dropdown-menu">
                     <!-- <li >
                        <router-link :to="{ name: 'EditTresoreriePage', params: { id:tresorerie.id } }" 
                    ...
                    <li >
                        <a
                          class="dropdown-item d-flex align-items-center" href="javascript:void(0);" @click="suppression(tresorerie.id,tresoreries,'tresoreries',`Tresorerie ${tresorerie.id}`)">
  ...
  export default defineComponent({
    ...
    setup(){
      ...
      const tresoreries = ref<Array<Tresorerie>>([]);   
      ...
      const handlePaginate = ({ page_, limit_ }) => {
        try {
          page.value = page_;
          getAllTresoreries(page_, limit_);
        } catch (error) {
          //
        }
      };
      ...
      function getAllTresoreries(page = 1, limi = 10, searchTerm = '') {
        return ApiService.get(`all/tresoreries?page=${page}&limit=${limi}&mot=${searchTerm}&`)
          .then(({ data }) => {
            tresoreries.value = data.data.data;
      ...
      function moddifier(Edittresoreries:Tresorerie) {
        tresorerie.value = Edittresoreries;
      }
  
      const deleteTresorerie = (id: number) => {
        ApiService.delete(`/tresoreries/${id}`)
        .then(({ data }) => {
          Swal.fire({
            text: data.message,
            toast: true,
        ...
        for(let i = 0; i < tresoreries.value.length; i++) {
          if (tresoreries.value[i].id === id) {
             tresoreries.value.splice(i, 1);
          }
        }
      };
      ...
      const total = () => {
      return tresoreries.value.reduce((sum, tresorerie) => sum + tresorerie.montant, 0);
    };
