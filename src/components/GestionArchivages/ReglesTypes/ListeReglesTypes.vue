<template>
  <div class="card mb-25 border-0 rounded-0 bg-white letter-spacing">
    <div
      ...
      <div class="d-sm-flex align-items-center">
        <a class="btn btn-primary" href="#" data-bs-toggle="modal" data-bs-target="#AddReglesTypesModal">
    ...
    <div class="card-body p-15 p-sm-20 p-md-25">
      <div class="table-responsive">
        <table class="table text-nowrap align-middle mb-0">
          <thead>
            <tr>
              <th scope="col" class="text-uppercase fw-medium shadow-none text-body-tertiary fs-13 pt-0">Regle conservation
              </th>
              <th scope="col" class="text-uppercase fw-medium shadow-none text-body-tertiary fs-13 pt-0">Type document
              </th>
              ...
              <td class="shadow-none lh-1 fw-medium text-black pe-0">
                ...
                <ul class="dropdown-menu dropdown-block"
                  ...
                  <li class="dropdown-item d-flex align-items-center">
                    <a href="javascript:void(0);" @click="moddifier(regleTypes)">
  ...
  <AddReglesTypesModal @get-all-regleTypes="getAllRegleTypes" :id="idreglesTypes"
    @openmodal="showModalEdite" @close="recharger" @refreshReglesTypes="refreshReglesTypes" />
</template>
<script lang="ts">
import { defineComponent, onMounted, ref  } from "vue";
import AddReglesTypesModal from "./AddReglesTypesModal.vue";
...
export default defineComponent({
  ...
  setup: () => {
    ...
    const reglesTypes = ref<Array<ReglesTypes>>([]);
    const idReglesTypes = ref(0);
    ...
    const handlePaginate = ({ page_, limit_ }: { page_: number, limit_: number }) => {
      try {
        page.value = page_;
        getAllReglesTypes(page_, limit_);
      } catch (error) {
        //
      }
    };
    ...
    function rechercher() {
      getAllReglesTypes(page.value, limit.value, searchTerm.value);
    }
    ...
    function moddifier(EditReglesTypes: ReglesTypes) {
      idReglesTypes.value = EditReglesTypes.id;
    }

    function showModalEdite(model: any) {
      showModal(model);
      idReglesTypes.value = 0;
    }

    const privileges = ref<Array<string>>(JwtService.getPrivilege());

    const checkReglesTypes = (name:string) => {
